...
CREATE TABLE IF NOT EXISTS Cards (
    ...
    Customer_ID INT REFERENCES Personal_Information(Customer_ID) NOT NULL
);
-- Create the SKU_Groups table
CREATE TABLE IF NOT EXISTS SKU_Groups (
    Group_ID SERIAL PRIMARY KEY,
    Group_Name VARCHAR(50) NOT NULL
);
-- Create the Product_Grid table
CREATE TABLE IF NOT EXISTS Product_Grid (
    SKU_ID SERIAL PRIMARY KEY,
    SKU_Name VARCHAR(50) NOT NULL,
    Group_ID INT REFERENCES SKU_Groups(Group_ID) NOT NULL
);
-- Create the Stores table
CREATE TABLE IF NOT EXISTS Stores (
    Transaction_Store_ID SERIAL PRIMARY KEY,
    SKU_ID SERIAL REFERENCES Product_Grid(SKU_ID) NOT NULL,
...
CREATE TABLE IF NOT EXISTS Checks (
    ...
    SKU_ID INT REFERENCES Product_Grid(SKU_ID) NOT NULL,
